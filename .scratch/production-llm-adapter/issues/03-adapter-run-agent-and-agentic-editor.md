...
- **Writer and Reviewer stay single-shot `complete()`** — an author must not grade its own draft, and a verdict stage must not loop on its own judgement. The agent *proposes*; code *disposes*.
- Adds an agent `step_limit` knob to `HarnessConfig` with a sane default.

## Acceptance criteria

- [ ] The adapter's `run_agent` runs a `create_agent` tool loop, invokes a bound `ToolSpec`, honors `step_limit` via `recursion_limit`, and returns decodable JSON — asserted over the recorded HTTP transport (no real DeepSeek in CI); the Editor's revision goes out on the creative (V4 Pro) tier.
- [ ] The domain and `stages.py` import only ports and `ToolSpec`; LangChain stays behind the adapter.
- [ ] With `ScriptedLLM.run_agent` driving `check_guardrails`, the Editor loop revises until `evaluate_piece()` is clean; a Piece that cannot pass within `step_limit` still raises the existing budget-exceeded error and escalates, never silently ships.
- [ ] A test asserts the Editor agent's recorded `run_agent` request carries the assembled `piece_spec + voice` as `instructions`, **and** the `editor.judge` call inside `check_guardrails` also carries `piece_spec + voice` — a refactor that drops either fails red.
- [ ] The Writer (Draft) and Reviewer (QA) remain single-shot `complete()` calls.
- [ ] The agentic Edit stage behaves identically under both runtimes (dual-runtime parity extended).
