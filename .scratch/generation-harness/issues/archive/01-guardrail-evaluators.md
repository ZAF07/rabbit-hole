...
_(Prefactor — no pipeline yet.)_ The encoded-taste core that every later stage consumes, built first and in isolation so it is exhaustively unit-testable: the guardrail checks as **pure functions** `artifact → [violations]`, one family per guardrail spec.
...
- [x] Crafted fixtures assert the **specific** violation code: a Piece opening on a definition → FAIL A1; a pure-abstraction paragraph → FAIL B1; a banned-filler phrase → FAIL D3.
- [x] A shared-Topic-adjacency Connection → FAIL A1; a hook identical from any origin → FAIL B3.
- [x] A constellation with a dead-end → FAIL I4; a Connection to a missing Piece → FAIL I5; a Piece with no cross-Topic outbound → FAIL I6.
- [x] Tier-1 checks return **binary** pass/fail (no numeric scores on the hard invariants).
- [x] Evaluators are pure — same input always yields the same violation list; no network or disk.
