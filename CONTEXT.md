...
**Runtime**:
An execution engine that drives one generation run over the shared stage manifest and taste specs. There are two, and they never share an execution path — only the manifest, the specs, and the run workspace ([ADR 0010](docs/adr/0010-content-generation-pipeline-architecture.md), [ADR 0019](docs/adr/0019-claude-code-generation-runtime-and-shared-harness-cli.md)): the **production runtime** (a deterministic LangGraph engine authored by the DeepSeek model, behind the admin HTTP trigger) and the **Claude Code runtime** (the interactive `/new-constellation` conductor, Claude as the author). Both are forced through the **same shared seam** — the `harness` CLI's binary checks, verdict log, and atomic write — so neither can publish an out-of-contract constellation; the verdict record's `runtime` field is what lets the Distiller tell the two apart. "Same run, either runtime" is a property, not a hope.
_Avoid_: mode, backend, driver (bare — the in-process operator driver is one entry point to a runtime, not the runtime itself), pipeline (that is the shared *shape* both runtimes walk, not an engine).

**Conductor**:
The role that walks a run for the Claude Code runtime — the `/new-constellation` skill body, not a subagent or an agent card. It reads the manifest at run start, walks stages in order under the deliverable-on-disk gate, delegates each stage to its subagent, fires the three human gates, and calls the shared seam for every binary check, verdict, and the write. Claude does the creative and judgment work as conductor; the seam disposes. Distinct from a **Bounded-Worker Agent** (which makes one local decision *inside* a stage) and pointedly not a supervisor that overrides the deterministic gates.
_Avoid_: orchestrator, supervisor, agent (bare), the eighth agent (it is a skill persona, not a roster card).

**Verdict**:
One human gate action on one review target — **approve**, **edit-approve** (approve after editing the working copy; the machine→human diff is the learning signal), or **reject with a reason** ([ADR 0013](docs/adr/0013-human-review-surface-is-the-file-workspace.md)). Verdicts accumulate append-only per run and are the Distiller's raw material; each carries the **runtime** and model that produced the draft it judges.
