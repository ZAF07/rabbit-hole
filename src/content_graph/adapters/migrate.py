"""Forward-only, versioned migration runner for the Postgres store."""
...
_MIGRATIONS_PACKAGE = "content_graph.adapters.migrations"
...
def apply_migrations(conn: psycopg.Connection[Any]) -> list[str]:
    ...
    Each migration runs in its own transaction and is recorded on success,
    so a re-run applies only what is pending.
